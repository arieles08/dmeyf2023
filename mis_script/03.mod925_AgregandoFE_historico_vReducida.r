...
#--FUNCION PARA TENDENCIA en N-Ventanas temporal: habria que cambiarla para hacer como los LAGs y generar de a ventana x vez, porque demora muchisimo------------------------------------------------------------------------
#calcula la tendencia de las variables cols de los ultimos N meses definidos en ventanas
TendenciaYmuchomas <- function(dataset, cols, ventanas = c(6), tendencia = TRUE, 
...
#--FUNCION para GENERAR LAGS y DELTA_LAGS: Hay que correrlos individualmente -> cuelga el proceso despues de muchisima demora----------------------------------------------------
...
#FUNCION1: Genera TENDENCIA y ratioAvg, para ventana de 3, 6 y 9 meses #demora demasiado: tuve que reducirla en la 3ra corrida despues de errores de archivos dataset generados
TendenciaYmuchomas(dataset, cols_lagueables, ventanas = c(6), tendencia = TRUE, 
                   minimo = FALSE, maximo = FALSE, promedio = FALSE, ratioavg = TRUE, ratiomax = FALSE)
dim(dataset) #4562810    1402 -> 1399 (sin add_)(quedo viejo de la corrida anterior)
...
dim(dataset) #4562810     2599 (termino con 2599 variables la ultima version reducida, tenia 4359 la anterior) -> proxima competencia: correr pruning con canaritos
